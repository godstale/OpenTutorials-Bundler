...
2. `converted/<package-slug>/package-manifest.json` 작성 (통합 강좌 매니페스트)
   ...
   - `bundler_protocol_version`: `"1.3.1"` 필수
...
- `package-manifest.json` 에 `"bundler_protocol_version": "1.3.1"` 이 반드시 명시되어 있어야 하며, 필수 필드(`author` (nickname 필수, email/website 선택), `target_age`, `category`)가 정확히 포함되어야 함
...
- `license`가 `"custom"`이 아니어도, 제3자 원저작물(책·영상·이미지·오픈소스 등)을 각색·인용한 강좌라면 `license_file`을 선택적으로 첨부해 출처·라이선스·각색 범위·주의사항을 고지할 수 있음 (프로토콜 v1.3.1, `protocol/protocol.md` 4.1.2절)
