...
3. **필수 메타데이터 사용자 인터뷰**: `target_age`, `category`, `license`(제3자 원저작물 각색 여부 포함) 등 원고만으로 판단하기 어려운 필수/핵심 항목은 임의로 추정하지 말고 `protocol/creator-interview-guide.md`의 질문 양식을 참고해 사용자에게 확인한다. 제3자 원저작물을 각색·인용했다면 출처와 라이선스 조건을 확인하여 `license_file`(예: `LICENSE.md`)에 고지한다.
4. `converted/<course-name>/` 폴더 생성
5. 파일 순서대로 생성:
   - `package-manifest.json` — 강좌 및 패키지 메타데이터 (필수, 프로토콜 1.1.5 준수)
...
7. ZIP 생성 전 **번들 검증 체크리스트** 전체 항목 통과 확인 (아래 섹션 참조). `bundler_protocol_version`이 현재 프로토콜 버전과 일치하는지, 재생성 시 `version`이 갱신되는지 반드시 확인 ([C15] 참조)
...
10. 완료 보고 (생성된 카드 수, 체크포인트 위치, ZIP 경로)
...
2. `converted/<package-slug>/package-manifest.json` 작성 (통합 강좌 매니페스트)
   - **필수/권장 필드**: `title`, `slug`, `description`, `author`, `thumbnail`, `published`, `sequential_play`, `force_checkpoint`, `courses`, `version`, `changelog`, `bundler_protocol_version`, `target_age`, `category`, `language`, `tags`, `license`
   - `bundler_protocol_version`: `"1.1.5"` 필수
   ...
   - `license`: Optional. 사전 정의 값(`CC-BY-4.0`, `CC-BY-SA-4.0`, `CC-BY-NC-4.0`, `CC-BY-NC-SA-4.0`, `CC-BY-ND-4.0`, `CC-BY-NC-ND-4.0`, `CC0-1.0`, `all-rights-reserved`) 또는 `"custom"`. 생략 시 기본값 `"all-rights-reserved"`. `"custom"` 지정 시 `license_file`(ZIP 루트에 포함할 라이선스 전문 파일명) 필수. `custom`이 아니어도 제3자 원저작물을 각색한 경우 `license_file`로 출처/이용조건 고지 가능 (4.1.2절)
...
3. 패키지 검증 체크리스트 [P0]~[P6] 통과 확인 (버전/프로토콜 버전 확인은 [C15] 참조)
...
- `package-manifest.json` 에 `"bundler_protocol_version": "1.1.5"` 이 반드시 명시되어 있어야 하며, 필수 필드(`author` (nickname 필수, email/website 선택), `target_age`, `category`)가 정확히 포함되어야 함

### [C13] target_age / language 형식 검증 (프로토콜 v1.1.3)
- `target_age`: `all`(전연령), `x+`(x세 이상), `min-max`(연령대 범위) 형식만 허용. 자유 문자열 사용 금지
- `language`(선택): `"ko"` 또는 `"en"`만 허용, 생략 시 기본값 `"ko"`

### [C14] license / license_file 검증 (프로토콜 v1.1.4)
...
- `license`가 `"custom"`인 경우 `license_file`(예: `"LICENSE.md"`) 필드가 필수이며, 해당 파일이 실제로 존재하고 ZIP 루트에 포함되어야 함
- `license`가 `"custom"`이 아니어도, 제3자 원저작물(책·영상·이미지·오픈소스 등)을 각색·인용한 강좌라면 `license_file`을 선택적으로 첨부해 출처·라이선스·각색 범위·주의사항을 고지할 수 있음 (프로토콜 v1.1.5, `protocol/protocol.md` 4.1.2절)

### [C15] 프로토콜 버전 / 강좌 버전 확인 (필수 체크사항)
- `package-manifest.json`의 `bundler_protocol_version`이 `protocol/protocol.md`의 현재 `Version:` 값과 정확히 일치하는지 확인 (불일치 시 빌드 중단)
- 라이선스 변경 등 콘텐츠·메타데이터를 재생성할 때는 `version` 필드가 갱신되었는지 확인 (`build_course.py`/`build_package.py`는 기본적으로 패치 버전 자동 +1)
