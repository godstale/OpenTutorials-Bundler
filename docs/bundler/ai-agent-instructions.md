...
1. **프로토콜 버전 준수 및 명시**:
   - 모든 통합 강좌 패키지는 **Open Tutorials Course Bundler Protocol v1.0.0**을 준수해야 하며, 강좌 번들을 제작할 때 현재 사용한 프로토콜 버전도 반드시 명시해야 합니다.
...
- ZIP 패키징 전 아래 스크립트 로직을 머릿속으로 혹은 가상 실행하여 검증하십시오.
  ...
  - `package-manifest.json`에 `bundler_protocol_version`이 명시되었고 사용된 프로토콜 버전(예: `"1.0.0"`)과 일치하는가?
...
### [V1] 프로토콜 버전 명시
- [ ] `package-manifest.json`에 `"bundler_protocol_version": "1.0.0"` (또는 제작에 준수한 최신 프로토콜 버전)이 누락 없이 명시되어 있는가?
