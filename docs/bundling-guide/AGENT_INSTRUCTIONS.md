...
2. `cards/*.mdx` 또는 `cards/*.json`: 각 페이지의 본문. (1페이지 = 1 MDX 파일 또는 동영상 카드 JSON 파일). 마크다운과 컴포넌트 활용 가능. 이미지 경로는 로컬 경로(`../images/파일명.png` 등)로 작성.
...
3-b. **패키지 번들 생성 단계 (패키지 포함 시)**
   ...
   - **스키마 (통합 강좌 매니페스트)**:
     ...
     - `bundler_protocol_version` (string, 필수): 이 번들이 준수한 번들러 프로토콜 명세 버전 (예: `"1.0.0"`)
...
- **프로토콜 버전 명시 필수**: 강좌 번들 파일을 제작할 때 사용된 프로토콜 버전을 반드시 `package-manifest.json` 의 `bundler_protocol_version` 필드에 정확히 명시해야 합니다. (예: `"1.0.0"`)
