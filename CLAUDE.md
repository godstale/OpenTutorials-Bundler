...
- *발생 이력: protocol v1.1.2에서 target_age가 자유 문자열에서 규격화된 형식으로 변경됨*
...
- `license`가 `"custom"`이 아니어도, 강좌가 제3자 원저작물(책·영상·이미지·오픈소스 등)을 각색·인용한 경우에는 `license_file`을 선택적으로 첨부하여 출처·라이선스·각색 범위·주의사항을 고지할 수 있음 (프로토콜 v1.1.5, `protocol/protocol.md` 4.1.2절 참조). Open Tutorials 앱이 이 내용을 학습자에게 노출하므로, 원저작물을 각색한 강좌는 되도록 첨부를 권장
