...
4. 파일 순서대로 생성:
   ...
   - `config.json` — 메타데이터 + 카드 순서 + 체크포인트
     ...
     - 작성 완료 후 반드시 확인: **toc 트리의 모든 `filename` 값이 `cards[]`에 존재하고, `cards[]`의 모든 항목이 toc 트리의 leaf 노드에 존재하는가.** 불일치 시 PennyPress 등록 오류.
...
9. 완료 보고 (생성된 카드 수 = toc 수, 체크포인트 위치, ZIP 경로, preview 경로)

### ZIP 재생성

"<course-name> ZIP 다시 만들어줘" 요청 → 6단계만 재실행.
...
> **주의**: `Compress-Archive` PowerShell 명령은 `cards/` 등 하위 폴더 구조를 유지하지 못하고 모든 파일을 루트에 평탄화(flatten)한다. 반드시 Python `zipfile` 모듈을 사용해 폴더 구조를 보존해야 한다.
...
#### 패키지 생성 순서
1. 개별 강좌 번들 ZIP들 생성 완료 후
2. `packages/<package-slug>/package-manifest.json` 작성
3. 패키지 검증 체크리스트 [P1]~[P3] 통과 확인
4. `python tools/build_package.py <package-slug>` 실행
5. 완료 보고: 개별 강좌 ZIP 목록 + `converted/<package-slug>-pkg.zip` 경로
...
> **운영 규칙**: PennyPress 등록 또는 검증 중 새로운 오류가 발생하면, 원인을 파악한 즉시 아래 체크리스트에 항목을 추가한다. 오류는 반복되어서는 안 된다.
...
### [P0] 패키지 — courses 목록 비어있음 금지
- `package-manifest.json`의 `courses[]`가 빈 배열이거나 필드 자체가 없으면 안 됨
- `build_package.py`가 자동 검증하므로, 스크립트 오류 시 매니페스트 확인

### [P1] 패키지 — courses 슬러그 존재 확인
- `package-manifest.json`의 `courses[]`에 열거된 모든 슬러그가 `converted/` 폴더 내에 실제 존재해야 함
- `build_package.py`가 자동 검증하므로, 스크립트 오류 시 누락 슬러그 확인 후 수정

### [P2] 패키지 — courses 슬러그 중복 금지
- `courses[]` 내에 동일 슬러그가 두 번 이상 등장하면 안 됨

### [P3] 패키지 — thumbnail 파일 존재 확인
- `thumbnail` 필드가 정의된 경우, `packages/<package-slug>/<thumbnail>` 파일이 실제 존재해야 함
- 썸네일 없이 배포하려면 `thumbnail` 필드 자체를 생략
